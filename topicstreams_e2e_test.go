	...
	for time.Now().Before(deadline) {
		...
		case ps.eval <- func() {
			if gs, ok := ps.rt.(*GossipSubRouter); ok {
				enabled <- gs.extensions.topicStreamsNegotiated(pid)
			} else {
				enabled <- false
			}
		}:
	...
	tests := []struct {
	...
	}{
		{
			...
			send: func(ps *PubSub, s network.Stream) error {
				return topicstreams.WriteFrame(s, &pb.TopicRPCHeader{})
			},
		},
		{
			name: "empty RPC",
			send: func(ps *PubSub, s network.Stream) error {
				if err := topicstreams.WriteFrame(s, &pb.TopicRPCHeader{Topic: proto.String("topic")}); err != nil {
					return err
				}
				return topicstreams.WriteFrame(s, &pb.TopicRPC{})
			},
		},
	}
...
func TestTopicStreamsRejectControlStreamPayloads(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			psubs := []*PubSub{
				getGossipsub(ctx, hosts[0], WithTopicStreams()),
				getGossipsub(ctx, hosts[1], WithTopicStreams(), WithAppSpecificRpcInspector(func(peer.ID, *RPC) error { inspected.Add(1); return nil })),
			}
			connect(t, hosts[0], hosts[1])
			waitForTopicStreamsEnabled(t, psubs[1], hosts[0].ID())
			inspectedBefore := inspected.Load()
			...
			tc.payload(rpc)
			if err := topicstreams.WriteFrame(s, rpc); err != nil {
				t.Fatal(err)
			}

			waitForConnectionClosed(t, s.Conn())
			if inspected.Load() != inspectedBefore {
				t.Fatal("wrong-stream payload reached application inspector")
			}
		})
	}
}

func TestTopicStreamsAcceptUnnegotiatedInboundStream(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	hosts := getDefaultHosts(t, 2)
	sender := getGossipsub(ctx, hosts[0])
	receiver := getGossipsub(ctx, hosts[1], WithTopicStreams())
	if _, err := receiver.Subscribe("topic"); err != nil {
		t.Fatal(err)
	}
	if _, err := sender.Subscribe("control-topic"); err != nil {
		t.Fatal(err)
	}
	connect(t, hosts[0], hosts[1])
	...
	if err := topicstreams.WriteFrame(s, &pb.TopicRPC{Payload: &pb.TopicRPC_Publish{Publish: &pb.TopicScopedMessage{Data: []byte("payload")}}}); err != nil {
		t.Fatal(err)
	}
	// Topic-stream RPCs are accepted based on local support; the absence of a
	// peer advertisement is not itself a protocol violation.
	time.Sleep(250 * time.Millisecond)
	if hosts[0].Network().Connectedness(hosts[1].ID()) != network.Connected {
		t.Fatal("unnegotiated inbound topic stream closed the connection")
	}
}
...
func TestTopicStreamsCloseOnPeerUnsubscribe(t *testing.T) {
	synctestTest(t, func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		...
		// The publisher must also close when the receiving peer unsubscribes.
		subs[1].Cancel()
		time.Sleep(2 * time.Second)

		if hasStreamProto(hosts[0], hosts[1].ID(), TopicStreamsProtocolID) {
			t.Fatal("expected the topic stream to close after peer unsubscribe")
		}
	})
}
