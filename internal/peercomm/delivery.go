...
func newInboundDelivery(ctx context.Context, pid peer.ID, hook func(peer.ID, InboundEvent) bool, onFailure func()) *inboundDelivery {
	d := &inboundDelivery{ctx: ctx, peer: pid, hook: hook, onFailure: onFailure, queue: make(chan deliveryRequest, mailboxSize)}
	go d.run()
	return d
}

func (d *inboundDelivery) submit(event InboundEvent, reply chan bool) bool {
	if d.hook == nil {
		if reply != nil {
			reply <- true
		}
	...
	select {
	case d.queue <- deliveryRequest{event: event, reply: reply}:
		return true
	case <-d.ctx.Done():
		if reply != nil {
			reply <- false
